 ...
 * @author Milos
 */
public class FIgrac extends javax.swing.JDialog {

    private boolean isEdit;

    /**
     * Creates new form FIgrac
     */
    public FIgrac(java.awt.Frame parent, boolean modal, boolean isEdit) {
        super(parent, modal);
        this.isEdit = isEdit;
        initComponents();
        if (isEdit) {
    ...
    private void jbtnSacuvajActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnSacuvajActionPerformed
        ...
        Igrac igrac;
        try {
            igrac = kreirajIIzvrsiValidaciju(ime, prezime, jmbg, ulica, broj, mesto, uzrast, trener);
            if (isEdit) {
                igrac.setStatus(jCheckBoxStatus.isSelected());
                ...
                if (so.getStatus() == StatusOdgovora.OK) {
                    Kontroler.notifyAllObservers(igrac);
                    JOptionPane.showMessageDialog(this, "Sistem je zapamtio izmene", "Obaveštenje", JOptionPane.INFORMATION_MESSAGE);
                } else {
                    JOptionPane.showMessageDialog(this, "Sistem ne može da zapamti podatke o igraču", "Greška", JOptionPane.ERROR_MESSAGE);
                }
                ...
                ServerskiOdgovor so = KomunikacijaSaServerom.getInstanca().primiOdgovor();
                if (so.getStatus() == StatusOdgovora.OK) {
                    JOptionPane.showMessageDialog(this, "Sistem je zapamtio igrača", "Obaveštenje", JOptionPane.INFORMATION_MESSAGE);
                } else {
                    JOptionPane.showMessageDialog(this, "Sistem ne može da zapamti igrača", "Greška", JOptionPane.ERROR_MESSAGE);
                }
                resetujSvaPolja();
            }
    ...
    private Igrac kreirajIIzvrsiValidaciju(String ime, String prezime, String jmbg, String ulica, String broj, Mesto mesto, Uzrast uzrast, Trener trener) throws Exception {
        ...
        try {
            ...
            ServerskiOdgovor so = KomunikacijaSaServerom.getInstanca().primiOdgovor();
            if (so.getStatus() == StatusOdgovora.OK) {
                igraci = (List<Igrac>) so.getOdgovor();
            }
        ...
        for (Igrac igrac : igraci) {
            if (isEdit && igrac.getId() == igracId) {
                continue;
            }
            if (igrac.getJmbg().equals(jmbg)) {
                JOptionPane.showMessageDialog(this, "Igrač sa datim jmbgom već postoji", "Upozorenje", JOptionPane.WARNING_MESSAGE);
                validnaForma = false;
            }
        }
        ...
        return new Igrac(igracId, ime, prezime, jmbg, ulica, broj, mesto, uzrast, trener);

    }

    void postaviVrednostPoljima(Igrac i) {
        jtxtIme.setText(i.getIme());
        jtxtPrezime.setText(i.getPrezime());
        jtxtJmbg.setText(i.getJmbg());
        jtxtUlica.setText(i.getUlica());
        jtxtBroj.setText(i.getBroj());
        jcbMesto.setSelectedItem(i.getMesto());
        jcbUzrast.setSelectedItem(i.getUzrast());
        jcbTrener.setSelectedItem(i.getTrener());
        jCheckBoxStatus.setSelected(i.isStatus());
        igracId = i.getId();
    }
